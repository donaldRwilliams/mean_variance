...
At a posterior by prior plot to figure 2:

``` r

cors <- posterior_samples(fit_stroop, pars = "cor_")


post_samps <- cors$cor_ID__congruencyincongruent__sigma_congruencyincongruent

prior_samps <- rethinking::rlkjcorr(10000, K = 4, eta = 2)[,,1][,2]




dens_post <- density(post_samps, n = 10000)
dens_prior <- density(prior_samps, n = 10000)

# density at zero
dens_post <- approx(dens_post$x,dens_post$y, xout = c(0))$y
dens_prior <- approx(dens_prior$x,dens_prior$y, xout = c(0))$y
if(is.na( dens_post)) dens_post <- 0


dat_plot <- data.frame(samples = c(post_samps, prior_samps),
                       dist = rep(c("Posterior", "Prior"), each = length(post_samps)))



plot_2B <- ggplot(dat_plot, aes(x = samples,
                        group = dist,
                     fill = dist)) +
  ...
  theme_bw(base_family = "Times") +
  ylab("Density") +
  xlab("Marginal Distribution") +
  scale_fill_manual(values = c("#CC79A7", "#009E73"), name = "Distribution") +
  theme(
...
plot_2 <- plot_grid(plot_2A, plot_2B, nrow = 1)
plot_2
```
